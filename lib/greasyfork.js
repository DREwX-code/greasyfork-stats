...
export async function getUserStats(user) {
  ...
  const { username, scripts } = await fetchUserData(user);

  if (!scripts.length) {
    throw new GreasyForkError(
      "No public scripts found for this user.",
      404
    );
  }
...
async function fetchUserData(user) {
  const url =
    `${GREASYFORK_BASE_URL}/en/users/${encodeURIComponent(user)}.json`;
...
function getUsernameFromParameter(user) {
  const value = String(user).replace(/^\d+-?/, "");
